...
- **Aesthetics pass** — typography (larger base size, nicer font), spacing, smoother transitions/animations, overall feel. Light pass can land anytime; deep pass here. — Status: TBD (see triage §3). (Calendar shipped; deep aesthetics still outstanding.)
  - **Button system (consistency) — Built 2026-06-26.** Single source-of-truth `Button` component (`src/components/ui/button.tsx`): variants `primary` (amber `--accent`, retired the old dark-ink fill) · `secondary` · `quiet` (muted bordered toolbar) · `ghost` · `dashed` (add-affordances) · `danger` · `danger-ghost`; sizes `sm`/`md`; standard `rounded-md`, padding, `disabled:opacity-40`, shared `focus-visible` amber ring (new a11y win), optional leading-`icon` slot. Migrated ~40 inline action buttons across `page.tsx`, `trip-header.tsx`, trip `page.tsx`, `planning-panel.tsx`, `itinerary-table.tsx`, `packing-panel.tsx`, `checklist-panel.tsx`, `tasks-panel.tsx`. Fixes the split where "New Trip" was amber while "Create"/"Save"/"Plan" were dark-ink with drifting padding/radius. Verified: `tsc`+`lint` clean, amber primary confirmed via browser probe (`bg rgb(181,101,29)`).
    - **Follow-up (TBD):** stateful toggle/affordance controls were intentionally left out of the component (they carry selected-state or drag/drop/colored-banner semantics): right-panel **tabs**, map **category-filter pills** + **theme-menu items**, in-cell **delete ✕** / row hover-icons, the dnd **"+ Add item"** drop-target, and the blue auto-schedule **notice-banner** Undo/✕. A future "segmented/toggle" + "icon-button" component pass could unify these.
